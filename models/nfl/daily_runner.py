"""
daily_runner.py — NFL daily picks generator.
...
free equivalent historical stats source for NFL, so this runs on Elo alone:
...
7. Write data/picks_nfl.json
8. Save Elo state

Always writes data/picks_nfl.json and exits 0, even on API failures — the
...
# This script lives at models/nfl/daily_runner.py, two directories below the
# repo root (models/nfl -> models -> repo root), so REPO_ROOT needs three
# dirname() calls — same pattern (and same past bug, see PR #8 on the MLB
# runner) that must be gotten right here too.
REPO_ROOT   = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
PICKS_PATH  = os.path.join(REPO_ROOT, 'data', 'picks_nfl.json')
ELO_STATE   = os.path.join(REPO_ROOT, 'elo_state_nfl.pkl')
...
def write_picks_json(ev_bets, today_str, bankroll):
    ...
    print(f"picks_nfl.json written: {len(output['picks'])} singles, "
...
def write_no_data(reason):
    ...
    print(f"no_data picks_nfl.json written ({reason}) -> {PICKS_PATH}")
...
def run_pipeline():
    ...
    print(f"[NFL] Loading Elo state...")
    tracker = load_elo()

    print(f"[NFL] Updating Elo from {yesterday_str}...")
    tracker = update_elo_from_yesterday(tracker, yesterday_str)

    print(f"[NFL] Fetching today's schedule ({today_str})...")
    todays_games = fetch_games_by_date(today_str)
    if not todays_games:
        tracker.save(ELO_STATE)
        write_no_data(f"no NFL games scheduled for {today_str} (likely off-season)")
        return

    predictions = build_predictions(tracker, todays_games)
    if not predictions:
        tracker.save(ELO_STATE)
        write_no_data(f"no upcoming NFL games for {today_str}")
        return

    print(f"[NFL] Fetching live odds...")
    ...
    tracker.save(ELO_STATE)
    print("[NFL] Done. Elo state saved.")
